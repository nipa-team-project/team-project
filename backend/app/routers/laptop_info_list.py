...
def read_laptops(page: int = 1, rating: str = None, db: Session = Depends(db.session)):
    try:
        laptops = crud.get_laptops_desc(db, page=page, rating=rating)
...
def read_laptops(page: int = 1, rating: str = None, db: Session = Depends(db.session)):
    try:
        laptops = crud.get_laptops_asc(db, page=page, rating=rating)
        if laptops is None or len(laptops) == 0:
            raise ValueError
        return laptops
    except ValueError:
